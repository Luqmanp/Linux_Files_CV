...
library(mnormt)
set.seed(1)
setwd("/u/l/u/luqman/stat761/data/nhtsa/cv/pass")
system("sh cleancode.sh > log.txt")
############ read data into R
bar <- read.csv("barclean.psv",sep="|",header=TRUE,na.strings=c("",999,9999,99999))
occ <- read.csv("occ02clean.psv",sep="|",header=TRUE,na.strings=c("",999,9999,99999,100000))
rest <- read.csv("rest02clean.psv",sep="|",header=TRUE,na.strings="")
...
write.csv(merged_guide,"merged.csv",row.names=FALSE)
write("merged_nhtsa.txt","merged.dsc")
write("NA",file="merged.dsc",append=TRUE)
write("2",file="merged.dsc",append=TRUE)
role[k+1] <- "w"
write.table(cbind(1:(k+1),c(names(merged_guide),"wt"),role),"merged.dsc",row.names=FALSE,col.names=FALSE,quote=FALSE,append=TRUE)
...
for (i in 0:v)
{
  ...
  wt[train] <- 1
  ptm <- proc.time()
  write.table(cbind(merged_guide[,-(k+1)],wt),"merged_nhtsa.txt",row.names=FALSE,quote=TRUE)
  system("guide < merged_nhtsa.in > logtree.txt")
  tmp <- read.table("merged_nhtsa.fit",header=TRUE)
  guidetree.err <- guidetree.err+sum(tmp$observed[test] != tmp$predicted[test])
  x<-proc.time() - ptm
  guidetree.time <- guidetree.time + x[3]

  ptm <- proc.time()
  system("guide < merged_nhtsa_forest.in > logforest.txt")
  tmp.forest <- read.table("merged_nhtsa_forest.fit",header=TRUE)
...
write.csv(glmnet.err/v,"glmnet.txt",row.names=FALSE)
write.csv(ctree.err/v,"ctree.txt",row.names=FALSE)
write.csv(rpart.err/v,"rpart.txt",row.names=FALSE)
write.csv(guidetree.err/v,"guidetree.txt",row.names=FALSE)
write.csv(guideforest.err/v,"guideforest.txt",row.names=FALSE)
write.csv(rF.err/v,"rF.txt",row.names=FALSE)
write.csv(lmerr/v,"lmerr.txt",row.names=FALSE)
write.csv(lmnet.con.err/v,"lmnetcon.txt",row.names=FALSE)
write.csv(ldamod.err/v,"ldamod.txt",row.names=FALSE)
write.csv(qdamod.err/v,"qdamod.txt",row.names=FALSE)
write.csv(svmmod.err/v,"svmmod.txt",row.names=FALSE)

write.csv(glmnet.time/v,"glmnet_time.txt",row.names=FALSE)
write.csv(ctree.time/v,"ctree_time.txt",row.names=FALSE)
write.csv(rpart.time/v,"rpart_time.txt",row.names=FALSE)
write.csv(guidetree.time/v,"guidetree_time.txt",row.names=FALSE)
write.csv(guideforest.time/v,"guideforest_time.txt",row.names=FALSE)
write.csv(rF.time/v,"rF_time.txt",row.names=FALSE)
write.csv(lmerr.time/v,"lmerr_time.txt",row.names=FALSE)
write.csv(lmnet.con.time/v,"lmnetcon_time.txt",row.names=FALSE)
write.csv(ldamod.time/v,"ldamod_time.txt",row.names=FALSE)
write.csv(qdamod.time/v,"qdamod_time.txt",row.names=FALSE)
write.csv(svmmod.time/v,"svmmod_time.txt",row.names=FALSE)
