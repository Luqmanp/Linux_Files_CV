...
write.table(cbind(1:k,names(merged_guide),role),file="merged.dsc",append=TRUE,row.names=FALSE,col.names=FALSE,quote=FALSE)
...
for (i in 0:v)
{
  test <- merged_no$gp_ind ==2
  train <-merged_no$gp_ind != 2
  ...
  train.numeric <- missForest(train.df.m[names_numeric])
  ...
  combi_test_train_numeric_imp <- missForest(combi_test_train_numeric)
  ...
  fit.rF <- randomForest(HIC2 ~ ., data=combi_train_imp_data, importance=TRUE, ntree=2000)
