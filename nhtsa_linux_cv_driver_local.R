...
library(mnormt)
set.seed(1)
setwd("/Users/luqmanpunekar/Documents/MBA/Sem4/Decision Trees/nhtsa/Code_for_merge/Linux_Files_CV")
...
names(merged_maked[,sapply(miss_impute, is.factor)])
#imputation for factor columns
miss_impute_factor <- miss_impute[,sapply(miss_impute, is.factor)]
miss_impute_numeric <- miss_impute[,sapply(miss_impute, is.numeric)]
...
levels(miss_impute_factor$ENGINE)

miss_impute_numeric$CARANG <- NULL
miss_impute_numeric_no <- miss_impute_numeric[!is.na(miss_impute_numeric),]

comp_data <- miss_impute_numeric[complete.cases(miss_impute_numeric),]
anyNA(comp_data)
cor(comp_data)

install.packages("corrplot")
library(corrplot)
xy<-cor(!is.na(miss_impute_numeric))
write.csv(xy,"corr.csv")
data.new <- data[,x,2,function(x) any(x > 0.99))]
data.new <- data[,!apply(xy,2,function(x) any(x > 0.99))]
head(data.new)
...
for (i in 0:v)
{
  test <- merged_no$gp_ind ==1
  train <-merged_no$gp_ind != 1
  ...
  test.df.m_imp <- combi_test_train[combi_test_train$gp==1,]
  ...
  pdf("fit_rpart.pdf",width=8, height=8)
  ...
  pdf("ctree_driver.pdf",width=20, height=10)
  plot(fit.ctree)
  dev.off()
...
!is.na(miss_impute_numeric)
datMy.scale<- scale(comp_data[2:ncol(comp_data)],center=TRUE,scale=TRUE);
